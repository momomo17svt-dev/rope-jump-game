...
React Native (Expo) フロントエンド + Go バックエンド（Render Web Service + Neon Postgres）の構成。
...
| バックエンド | Go |
| インフラ | Docker / Render |
| DB（本番） | PostgreSQL (Neon、永続無料 3GB) |
...
- DB は Neon を別管理。Render dashboard の `DATABASE_URL` env に Neon の pooled 接続文字列を設定する。pgx/v5 は `DefaultQueryExecMode=Exec` で transaction-mode 対応済み。
...
```
DATABASE_URL=postgresql://user:password@ep-xxxx-pooler.ap-southeast-1.aws.neon.tech/jumprope?sslmode=require
PORT=8080
```
